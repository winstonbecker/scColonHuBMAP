# Script to analyze ileum scRNA epithelail data
# WRB 2022
...
sample_name <- "ileum" # used as label for saving plots
analysis_parent_folder <- paste0(parent_folder, "scRNA_reproduce/epithelial/ileum/decontx_sctransform_cca/") # folder for this analysis
...
ileum <- readRDS(paste0(parent_folder, "scRNA_reproduce/all_cells/decontx_norm_scale_harmony/ileum_epithelial_samples_initial.rds"))
...
ileum[["percent.ribo"]] <- PercentageFeatureSet(ileum, pattern = "^RP[SL]")
rna.list <- SplitObject(object = ileum , split.by = "orig.ident")
...
rna.anchors <- FindIntegrationAnchors(object.list = rna.list, normalization.method = "SCT",
    anchor.features = features, dims = 1:30, reference = c(1,3)) 
...
cM <- confusionMatrix(rna.combined.sct$seurat_clusters, rna.combined.sct$orig.ident)
cM <- cM[order(as.numeric(rownames(cM))),]
...
RNA_markers_spec <- RNA_markers[RNA_markers$either,]
RNA_markers_spec[RNA_markers_spec$gene %in% rna.combined.sct.markers[rna.combined.sct.markers$cluster == 25,]$gene,"ident"]

DefaultAssay(rna.combined.sct.new) <- "integrated"
bad_clusters <- c(23,25)
set.seed(1)
# Subset
rna.combined.sct.new <- subset(rna.combined.sct.new, subset = seurat_clusters %ni% bad_clusters)
rna.combined.sct.new <- RunPCA(rna.combined.sct.new, verbose = TRUE)
rna.combined.sct.new <- RunUMAP(rna.combined.sct.new, reduction = "pca", dims = 1:30)
plotUMAP(rna.combined.sct.new, reduction = "umap", save_name = "rna_combined_sct_post_filter2")
set.seed(1)
rna.combined.sct.new <- FindNeighbors(rna.combined.sct.new, reduction = "pca", dims = 1:30)
rna.combined.sct.new <- FindClusters(rna.combined.sct.new, resolution = 3.0)
plotUMAP(rna.combined.sct.new, reduction = "umap", save_name = "rna_combined_sct_ref1_new_clust_post_filter2")
...
# cluster identification
  ...
  pdf(paste0("./umapharmony_cell_type_id.pdf"), width = 6, onefile = F)
  DimPlot(rna.combined.sct.new, group.by = "CellType", reduction = "umap", cols = pal) + theme_ArchR()
  dev.off()

  saveRDS(rna.combined.sct.new, file = "ileum.rds")
