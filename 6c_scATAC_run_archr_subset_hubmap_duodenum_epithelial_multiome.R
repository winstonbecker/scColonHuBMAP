...
new_project_save_name = "HuBMAP_duodenum_epithelial_cells_multiome"
subscript = "epithelial_duodenum"
...
if (0 %in% execute_steps){
    ...
    idxSample <- BiocGenerics::which((proj$Sample %in% unique(getCellColData(proj)$Sample)[order(unique(getCellColData(proj)$Sample))][27:71]) & (proj$Location %in% c("Duodenum")) & (proj$CellTypeRNA %in% epi_cell_types))
...
if (1 %in% execute_steps){
    ...
    proj_stromal <- addClusters(
        ...
        resolution = 0.5, force=TRUE, nOutlier = 10, seed = 1, maxClusters = 40
    )
...
duodenum_diet <- DietSeurat(duodenum, assays = c("decontXcounts"))
duodenum_diet <- subset(duodenum_diet, subset = orig.ident %in% multiome)
duodenum.data.full <- GetAssayData(object = duodenum_diet, slot = "counts")

rnaRowRanges <- readRDS("10x_RNA_row_ranges.rds")

duodenum.data.full <- duodenum.data.full[rownames(duodenum.data.full) %in% names(rnaRowRanges),]
library(stringr)
colnames(duodenum.data.full) <- str_replace(colnames(duodenum.data.full), "_", "#")

rowRangesSE <- rnaRowRanges[names(rnaRowRanges) %in% rownames(duodenum.data.full),]
duodenum.data.full <- duodenum.data.full[names(rowRangesSE),] # make sure they have the same order

seRNA<-SummarizedExperiment(assays=list(counts=duodenum.data.full), rowRanges= rowRangesSE) # create summarized experiment for adding to arrow
library(parallel)
