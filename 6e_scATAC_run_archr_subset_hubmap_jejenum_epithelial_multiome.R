...
new_project_save_name = "HuBMAP_jejunum_epithelial_cells_multiome"
subscript = "epithelial_jejunum"
...
idxSample <- BiocGenerics::which((proj$Sample %in% unique(getCellColData(proj)$Sample)[order(unique(getCellColData(proj)$Sample))][27:71]) & (proj$Location %in% c("Mid-jejunum","Proximal-jejunum")) & (proj$CellTypeRNA %in% epi_cell_types))
...
plotPDF(p7, name = paste(paste("Plot-UMAP2Harmony-CellType", subscript, sep = "-"), ".pdf", sep = ""), ArchRProj = proj_stromal, addDOC = FALSE, width = 5, height = 5)
...
proj_stromal <- addClusters(
    ...
    resolution = 0.3, force=TRUE, nOutlier = 20, seed = 1, maxClusters = 50
)
...
jejunum_diet <- DietSeurat(jejunum, assays = c("decontXcounts"))
jejunum_diet <- subset(jejunum_diet, subset = orig.ident %in% multiome)
jejunum.data.full <- GetAssayData(object = jejunum_diet, slot = "counts")

rnaRowRanges <- readRDS("10x_RNA_row_ranges.rds")

jejunum.data.full <- jejunum.data.full[rownames(jejunum.data.full) %in% names(rnaRowRanges),]
library(stringr)
colnames(jejunum.data.full) <- str_replace(colnames(jejunum.data.full), "_", "#")

rowRangesSE <- rnaRowRanges[names(rnaRowRanges) %in% rownames(jejunum.data.full),]
jejunum.data.full <- jejunum.data.full[names(rowRangesSE),] # make sure they have the same order

seRNA<-SummarizedExperiment(assays=list(counts=jejunum.data.full), rowRanges= rowRangesSE) # create summarized experiment for adding to arrow
library(parallel)
...
p1 <- plotEmbedding(proj_stromal, colorBy = "cellColData", name = "CellTypeRNA", embedding = paste0("UMAPHarmony", subscript), pal = pal, size = 0.3)
...
library(parallel)
proj_stromal <- addPeakSet(ArchRProj = proj_stromal, peakSet = peakSet)
...
p <- plotPeak2GeneHeatmap(ArchRProj = proj_stromal, groupBy = "CellTypeRNA")
plotPDF(p, name = paste(paste("Peak2-Marker-Heatmap", subscript, sep = "-"), ".pdf", sep = ""), width = 8, height = 8, ArchRProj = proj_stromal, addDOC = FALSE)
saveArchRProject(ArchRProj = proj_stromal, load = FALSE, overwrite = FALSE)
