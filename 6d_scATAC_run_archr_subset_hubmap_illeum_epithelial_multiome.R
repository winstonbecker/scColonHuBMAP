...
new_project_save_name = "HuBMAP_ileum_epithelial_cells_multiome"
subscript = "epithelial_ileum"
...
# the B009 ileum sample did not look good, so removing that
idxSample <- BiocGenerics::which((proj$Sample %in% unique(getCellColData(proj)$Sample)[order(unique(getCellColData(proj)$Sample))][27:71]) & 
    (proj$Location %in% c("Ileum")) & 
    (proj$CellTypeRNA %in% epi_cell_types) &
    (proj$Sample %ni% c("B009-A-301")))
...
proj_stromal <- addClusters(
    ...
    resolution = 0.5, force=TRUE, nOutlier = 10, seed = 1, maxClusters = 50
)

p3 <- plotEmbedding(ArchRProj = proj_stromal, colorBy = "cellColData", name = "Sample", size = 0.4, embedding = paste("UMAPHarmony", subscript, sep = ""))
p5 <- plotEmbedding(ArchRProj = proj_stromal, colorBy = "cellColData", name = "Location", size = 0.4, embedding = paste("UMAPHarmony", subscript, sep = ""))
p6 <- plotEmbedding(ArchRProj = proj_stromal, colorBy = "cellColData", name = "Donor", size = 0.4, embedding = paste("UMAPHarmony", subscript, sep = ""))
p7 <- plotEmbedding(ArchRProj = proj_stromal, colorBy = "cellColData", size = 0.4, name = paste("ClustersHarmony", subscript, sep = ""), embedding = paste("UMAPHarmony", subscript, sep = ""))
...
p6 <- plotEmbedding(ArchRProj = proj_stromal, colorBy = "cellColData", name = "CellTypeRNA", size = 0.4, embedding = paste("UMAPHarmony", subscript, sep = ""))
...
plotPDF(p6, name = paste(paste("Plot-UMAP2Harmony-CellTypeRNA-Harmony", subscript, sep = "-"), ".pdf", sep = ""), ArchRProj = proj_stromal, addDOC = FALSE, width = 5, height = 5)
...
ileum_diet <- DietSeurat(ileum, assays = c("decontXcounts"))
ileum_diet <- subset(ileum_diet, subset = orig.ident %in% multiome)
ileum.data.full <- GetAssayData(object = ileum_diet, slot = "counts")

ileum.data.full <- ileum.data.full[rownames(ileum.data.full) %in% names(rnaRowRanges),]
library(stringr)
colnames(ileum.data.full) <- str_replace(colnames(ileum.data.full), "_", "#")

rowRangesSE <- rnaRowRanges[names(rnaRowRanges) %in% rownames(ileum.data.full),]
ileum.data.full <- ileum.data.full[names(rowRangesSE),] # make sure they have the same order

seRNA<-SummarizedExperiment(assays=list(counts=ileum.data.full), rowRanges= rowRangesSE) # create summarized experiment for adding to arrow
library(parallel)
...
proj_stromal <- addBgdPeaks(proj_stromal, force = TRUE)
...
proj_stromal <- addImputeWeights(proj_stromal, reducedDims = paste0("Harmony", subscript), k=9, sampleCells = floor(nCells(proj_stromal)/5))
...
plotPDF(p1, name = "Plot-AbsorptiveU-Genes-Traj-UMAP.pdf", ArchRProj = proj_stromal, addDOC = FALSE, width = 5, height = 5)


trajMM  <- getTrajectory(ArchRProj = proj_stromal, name = "AbsorptiveU", useMatrix = "VierstraMatrix", log2Norm = FALSE)
p1 <- plotTrajectoryHeatmap(trajMM, pal = paletteContinuous(set = "solarExtra"))
trajGSM <- getTrajectory(ArchRProj = proj_stromal, name = "AbsorptiveU", useMatrix = "GeneExpressionMatrix", log2Norm = TRUE)
